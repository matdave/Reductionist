...
class RImage extends RAbstractImage
{
    ...
    public function crop(PointInterface $start, BoxInterface $size)
    {
        ...
        try {
            ...
            throw new RuntimeException("Gmagick: Crop operation failed. {$e->getMessage()}", $e->getCode(), $e);
        }
        $this->size = array($width, $height);
    ...
    protected function load($size = null)
    {
        try {
            ...
            throw new RuntimeException("Gmagick: Unable to open image {$this->filename}. {$e->getMessage()}", $e->getCode(), $e);
        }
        if ($this->format === IMG_JPG && $size !== null) {
