...
class Reductionist
{
    ...
    static public function startPoint($opt, $containerDims, $imageDims)
    {
        ...
        if ($x < 0) {
            $x = 0;
        }  // a negative value wouldn't make sense
        if ($y < 0) {
            $y = 0;
        }
     ...
     */
    public function processImage(string $input, string $output, array $options = array()): bool
    {
        if ($this->debug) {
        ...
        try {
            ...
            if (isset($scBox)) {
                ...
                if ($scale <= 0.5 && $this->gLib && $image->getFormat() === IMG_JPG) {
                    ...
                    $scStart = new Point(round($cropStartX * $scale), round($cropStartY * $scale));
                    $scBox = new Box(round($scBox->getWidth() * $scale), round($scBox->getHeight() * $scale));
                } else {
                    $scStart = new Point($cropStartX, $cropStartY);
                }
            ...
            if ($hasBG = ((isset($options['bg']) && !$outputIsJpg) || isset($farBox))) {
                ...
                $image = $this->imagine
                    ...
                    ->paste($this->gLib ? $image->getImage() : $image, isset($farPoint) ? $farPoint : new Point(0, 0));
            }
